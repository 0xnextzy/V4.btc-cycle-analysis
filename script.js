...
function getRiskZone(score) {
    if (score <= 30) return { zone: 'Accumulation Zone', class: 'low' };
    if (score <= 70) return { zone: 'Neutral Zone', class: 'medium' };
    return { zone: 'Overheated Zone', class: 'high' };
}
...
function updateHeroSection(btcData) {
    if (!btcData) return;
    
    const marketData = btcData.market_data;
    
    // Price
    document.getElementById('heroPrice').textContent = formatPrice(marketData.current_price.usd);
    ...
    changeElement.className = 'price-change-badge ' + (change24h >= 0 ? 'positive' : 'negative');
    
    // Volume
    document.getElementById('heroVolume').textContent = formatPrice(marketData.total_volume.usd);
    
    // Market Cap
    document.getElementById('heroMarketCap').textContent = formatPrice(marketData.market_cap.usd);
}

function updateQuickStats(btcData, globalData) {
    if (!btcData) return;
    ...
    if (globalData?.data) {
        ...
        document.getElementById('quickDominance').textContent = formatPercentage(dominance, 1);
    }
    
    // Rank
    document.getElementById('quickRank').textContent = btcData.market_cap_rank || '-';
    
    // Supply
    document.getElementById('quickSupply').textContent = formatSupply(marketData.circulating_supply);
}
...
function updateLayer1(btcData) {
    ...
    fromATHElement.className = 'metric-value ' + (fromATH >= 0 ? 'text-green' : 'text-red');
...
function updateRiskScore() {
    ...
    riskZoneElement.className = 'metric-label risk-zone-label text-' + 
        (riskInfo.class === 'low' ? 'green' : riskInfo.class === 'medium' ? 'yellow' : 'red');
}
...
function updateLayer7(fearGreedData) {
    ...
    // Color coding
    if (value <= 30) {
        signalElement.className = 'metric-value text-green';
    } else if (value >= 70) {
        signalElement.className = 'metric-value text-red';
    } else {
        signalElement.className = 'metric-value text-yellow';
    }
}
...
async function fetchAllData() {
    ...
    try {
        ...
        if (bitcoin) {
            updateHeroSection(bitcoin);
            updateQuickStats(bitcoin, global);
